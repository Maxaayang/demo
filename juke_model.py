...
def manipuate_latent_space(piano_roll, vector_up_t, vector_high_d, vector_up_down_t,
    ...
    if with_input and piano_roll is not None:
        ...
        encoding = vae.encoders[0](piano_roll)
    else:
    ...
    encoding = encoding.squeeze(axis = 0)
    zs, xs_quantised, commit_losses, quantiser_metrics = vae.bottleneck(encoding.to('cuda'))
    # z, vq_loss = vae.vq_layer(encoding) 
    reconstruction = vae.decoders[0](xs_quantised)
    ...
    changed_reconstruction = vae.decoders[0](changed_z)
    ...
    changed_reconstruction = [item.cpu().detach().numpy() for item in reconstruction_new] 
