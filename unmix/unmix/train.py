...
from data.dataload import SequenceMIDI
...
def train(model, orig_model, opt, shd, scalar, ema, logger, metrics, X, hps):
    model.train()
    orig_model.train()
    ...
    # return loss, {key: metrics.avg(key) for key in _metrics.keys()}
    ...
    #     with t.no_grad():
    #         if log_input_output:
    #             log_inputs(orig_model, logger, x, y, x_out, hps)
...
def run(hps="teeny", port=29500, **kwargs):
    ...
    dataset = SequenceMIDI(hps.audio_files_dir, 25)
    ...
    for epoch in range(hps.curr_epoch, hps.epochs):
        ...
        if hps.train:
            for batch,(X, y) in enumerate(tqdm(loader)):
                loss, train_metrics = train(model, model, opt, shd,
                                    scalar, ema, logger, metrics, X, hps)
        ...
        if rank == 0:
            print('Train', ' '.join(
                [f'{key}: {val:0.4f}' for key, val in train_metrics.items()]))
        if (epoch+1) % 10 == 0:
            t.save(model.state_dict, "./model/model%d.pth" % (epoch+1))
            print("Done!")
        ...
        if hps.test:
            ...
            test_metrics = evaluate(
                model, model, logger, metrics, data_processor, hps)
